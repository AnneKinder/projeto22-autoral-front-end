...
Front-end for when, an event management solution.

## About

when is a web browser application with which you can manage every single aspect of your event.
...
3. Populate `.env` file based on `.env.example`. `REACT_APP_API_BASE_URL` should point to your API server (when-back)
...
Set `POSTGRES_HOST` to `when-postgres-development` for `.env.development` and `when-postgres-test` for `.env.test` file. It is the name of the postgres container inside docker-compose file. Docker Compose will start the postgres container for you, create the database and host alias for you.
